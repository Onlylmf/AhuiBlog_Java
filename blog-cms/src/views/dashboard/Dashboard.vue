...
<script>
	...
	export default {
		...
		methods: {
			getData() {
				getDashboard().then(res => {
					if (res.code === 200) {
						...
						this.initTagEcharts()
						//渲染访客地图数据
            console.log(res.data.cityVisitor, "---Onlylmf")
						let mapData = this.convertData(res.data.cityVisitor)
            console.log(mapData, "---Onlylmf233")
			...
			convertData(data) {
        console.log(data, "---Onlylmf_data")
				let res = []
				for (let i = 0; i < data.length; i++) {
					let geoCoord = geoCoordMap[data[i].city]
          console.log(geoCoordMap['上海'], "---Onlylmf_geoCoord")
					if (geoCoord) {
						res.push({
							name: data[i].city,
							value: geoCoord,
