<template>
	<div ref="nav" class="ui fixed inverted stackable pointing menu" :class="{'transparent':$route.name==='home' && clientSize.clientWidth>768}">
		<div class="ui container">
			<router-link to="/">
				<h3 class="ui header item m-blue"><img src="../../../public/img/logoNav.gif" height="134" width="250"/>{{ blogName }}</h3>
			</router-link>
      ...
      <el-dropdown trigger="click" @command="pictuerRoute">
				<span class="el-dropdown-link item" :class="{'m-mobile-hide': mobileHide,'active':$route.name==='category'}">
					<i class="picture icon"></i>图床<i class="caret down icon"></i>
				</span>
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item command="setting">配置</el-dropdown-item>
          <el-dropdown-item command="manage">管理</el-dropdown-item>
          <el-dropdown-item command="help">教程</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
...
<script>
	...
	export default {
      ...
      pictuerRoute(name) {
				this.$router.push(`/pictureHosting/${name}`)
			},
